...
case class Pile(tilepile: List[Card]) extends PileInterface {
  def this() = this(List.fill(22)(Card("=")) ::: List.fill(8)(Card("+")) ::: List.fill(8)(Card("-")) ::: List.fill(6)(Card("*"))
    ::: List.fill(6)(Card("/")) ::: List.fill(7)((0 to 9).toList).flatten.map(x => Card(x.toString)))

  def this(equal: Int, plusminus: Int, muldiv: Int, digit: Int) =
    this(List.fill(equal)(Card("=")) ::: List.fill(plusminus)(Card("+")) ::: List.fill(plusminus)(Card("-")) ::: List.fill(muldiv)(Card("*"))
      ::: List.fill(muldiv)(Card("/")) ::: List.fill(digit)((0 to 9).toList).flatten.map(x => Card(x.toString)))
